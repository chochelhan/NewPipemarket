...
export default {
    ...
    computed: {
        ...
        recePcsErrors() {
            ...
            !this.$v.recePcs.numeric && errors.push('휴대폰 번호는 숫자만 입력가능합니다')
            if(this.$v.recePcs.$model.length!=11) {
                errors.push('휴대폰 번호를 확인하세요')
            } else {
                if(this.$v.recePcs.$model.substring(0,3) != '010') {
                    errors.push('휴대폰 번호를 확인하세요')
                }
            }
            return errors
    ...
    methods: {
        validate() {
            this.$v.$touch();
            if(this.recePcsErrors.length>0) {
                return;
            }
            if (!this.$v.$anyError) {
