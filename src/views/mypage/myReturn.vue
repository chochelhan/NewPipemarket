    ...
    <div class="page-contents">
        <div style="background-color: #f1f1f1;margin-top:25px;">
            <div class="px-5 py-6 bg-white mb-3">
                <div class="text-base font-semibold mb-5">환불 상품을 선택해주세요.</div>
          ...
          "
            ...
            <div class="px-5 py-6 bg-white mb-3">
                <div class="flex items-center justify-between mb-5">
                    <div class="text-base font-semibold">반품 비용 ({{ deliveryPrice }}원)</div>
                    <img src="../../assets/icons/smallExclamationMark.svg"/>
                </div>
                <div class="flex mt-4" id="payment-1">
                    <img
                        src="../../assets/icons/radioUnChecked.svg"
                        alt=""
                        id="payment-icon-1"
                    />
                    <div class="text-15 ml-3">환불금에서 차감해주세요</div>
                </div>
                <div class="flex mt-4" id="payment-2">
                    <img
                        src="../../assets/icons/radioUnChecked.svg"
                        alt=""
                        id="payment-icon-2"
                    />
                    <div class="text-15 ml-3">지금 결제할게요.</div>
                </div>
            </div-->
            <div class="px-5 py-6 bg-white mb-3">
                <div class="text-base font-semibold mb-5">환불 상세 정보</div>
                <div class="flex items-center justify-between">
                    <p>총 환불 금액</p>
                    <p>{{ util.numberFormat(totalReturnPrice) }}원</p>
                </div>

                <div class="p-4 mt-2" style="background-color: #f1f1f1">
                    <div class="flex items-center justify-between mb-2">
                        <p>상품금액</p>
                        <p>{{ util.numberFormat(totalGoodsPrice) }}원</p>
                    </div>
                    <div class="flex items-center justify-between">
                        <p>반품 배송비</p>
                        <p>-{{ deliveryPrice }}원</p>
                    </div>
                    <div v-if="returnPriceMsg" style="font-size:11px;">
                        무료로 배송받은 경우 왕복 배송비가 청구됩니다
                    </div>
                </div>
                <div class="text-base font-semibold mb-5" style="margin-top:25px;">환불 방식</div>
                ...
                <div class="p-4 mt-2" style="background-color: #f1f1f1">
                    ...
                    <div class="flex items-center justify-between" v-if="returnCouponPrice>0">
                        <p style="min-width:100px;">쿠폰</p>
                        <p  style="text-align:right;">{{ returnCouponName }}</p>
                    </div>
                </div>
            ...
            <div
                ...
                @click="showGuide()"
            >
                <div class="text-base font-semibold">환불 안내사항</div>
            ...
            <div class="px-5 py-6 bg-white mb-3" style="color: #3b3b3b">
                <div class="flex justify-center items-center w-full" @click="agreeCheck()">
                    ...
                    <img v-else
                         ...
                         alt=""
                    />
                    <div class="ml-2">환불 안내사항을 확인하였습니다.</div>
                </div>
                <v-btn
                    depressed
                    rounded
...
export default {
    ...
    data() {
        return {
            ...
            totalReturnPrice: 0,
            totalGoodsPrice: 0,
            returnPriceMsg: false,
            refundComplete: false,
            orgDelivPrice: 0,
            orgTransPrice:0,
            orgTotalGoodsPrice:0,
            returnCouponName:'',
    ...
    methods: {
                ...
                if (res.data.result == 'success') {
                    ...
                    for (let key in res.data.data.order.orderDetail) {
                        for (let item of res.data.data.order.orderDetail[key]) {
                            item.checked = true;
                            totalGoodsPrice += item.dcprice * item.pcnt;
                            this.orderList.push(item);
                        }
                    }
                    this.totalGoodsPrice = totalGoodsPrice;
                    this.orgTotalGoodsPrice = totalGoodsPrice;
                    ...
                    if (res.data.data.transPrice) {
                        this.orgTransPrice = res.data.data.transPrice;
                        let delivPrice = 0;
                        if (res.data.data.order.deliveryPrice) {

                            for (let key in res.data.data.order.deliveryPrice) {
                                delivPrice = parseInt(res.data.data.order.deliveryPrice[key]);
                            }
                        }
                        this.refundComplete = res.data.data.refundComplete;
                        if (res.data.data.refundComplete) {
                            ...
                            this.orgDelivPrice = delivPrice;

                            if (delivPrice < 1) {
                                this.returnPriceMsg = true;
                                this.returnPrice = res.data.data.transPrice * 2;
                                this.send_deliveryPrice = res.data.data.transPrice * 2;
                                this.deliveryPrice = UTIL.numberFormat(this.returnPrice);
                            } else {
                    ...
                    this.cancelReasonList.push({value: '', text: '환불 사유를 선택하세요'});
                    ...
                    if (res.data.data.isClaimData && res.data.data.isClaimData.price) {
                        //parseInt(res.data.data.isClaimData.price);

                    }
                    if (res.data.data.order.couponIx && res.data.data.order.couponPrice) {
                        this.returnCoupon = true;
                        this.returnCouponIx = res.data.data.order.couponIx;
                        if (res.data.data.couponSaleType == '1') {

                            this.itemDisabled = true;
                        } else {
                            this.useCouponPrice = parseInt(res.data.data.order.couponPrice);
                        }
                        this.returnCouponPrice = parseInt(res.data.data.order.couponPrice);
                        this.returnCouponName = res.data.data.order.couponName;
                        totalGoodsPrice = parseInt(totalGoodsPrice) - parseInt(res.data.data.order.couponPrice);

                    }
                    let price = parseInt(totalGoodsPrice) - parseInt(this.returnPrice);
                    if (res.data.data.isClaimData && res.data.data.isClaimData.point) {
                        this.orgUsePoint = parseInt(res.data.data.isClaimData.point);
                        if (price > 0) {
                            this.returnPoint = parseInt(res.data.data.isClaimData.point);
                        } else {
                            this.returnPoint = parseInt(res.data.data.isClaimData.point) - this.returnPrice;
                        }

                    }
                    price = price - this.returnPoint;
                    if (price < 0) price = 0;

                    this.paymentInfo = {
                        price: UTIL.numberFormat(price),
                        //goodsPrice: UTIL.numberFormat(res.data.data.paymentInfo.total_listprice - res.data.data.paymentInfo.total_dc),
                    }
                    this.totalReturnPrice = price + this.returnPoint + this.returnCouponPrice;
        ...
        updateCancelInfo() {
            ...
            for (let item of this.orderList) {
                if (item.checked) {
                    let sd = item.dcprice * item.pcnt;
                    dcprice = dcprice + parseInt(sd);
                    cnt++;
                }
            }
            this.totalGoodsPrice = parseInt(dcprice);
            let price = parseInt(dcprice);
            const delivRemailPrice = parseInt(this.orgTotalGoodsPrice) - price;
            this.returnPriceMsg = false;
            if(!this.refundComplete) {
                if (this.orgDelivPrice < 1) {
                    if(delivRemailPrice < 30000) {
                        this.returnPriceMsg = true;
                        this.returnPrice = parseInt(this.orgTransPrice) * 2;
                        this.send_deliveryPrice = parseInt(this.orgTransPrice) * 2;
                        this.deliveryPrice = UTIL.numberFormat(this.returnPrice);
                    } else {

                        this.returnPrice = parseInt(this.orgTransPrice);
                        this.send_deliveryPrice = parseInt(this.orgTransPrice);
                        this.deliveryPrice = UTIL.numberFormat(this.returnPrice);
                    }
                }

            }
            price = price - this.returnPrice;
            this.returnPoint = 0;
            if (price>0 && this.orgUsePoint > 0) {
                let remainPoint = price - this.orgUsePoint;
                if (remainPoint < 1) {
                    this.returnPoint = price;
                    price = 0;

                } else {
                    price = remainPoint - this.returnPrice;
                    this.returnPoint = this.orgUsePoint;

                }
            }
            this.returnCouponPrice = 0;
            if (price > 0 && this.useCouponPrice > 0) {
                if (price >= this.useCouponPrice) {
                    this.returnCoupon = true;
                    price = price - this.useCouponPrice;
                    dcprice = parseInt(dcprice) - this.useCouponPrice;
                    this.returnCouponPrice = this.useCouponPrice;
                    this.returnCouponName = res.data.data.order.couponName;
                } else {
                    this.returnCouponName = '';
                    this.returnCoupon = false;
                }
        ...
        cancelSubmit() {
            ...
            /*if (this.resultPrice < 1) {
                let params = {message: '총 환불금액이 0원 입니다'};
                this.$eventBus.$emit('modalOpen', params);
                return;
            }
            ...
            frm.append('returnPoint', this.returnPoint)
            frm.append('resultPrice', this.resultPrice)
            if (this.returnCoupon) frm.append('returnCouponIx', this.returnCouponIx);
            frm.append('status', this.status)
            frm.append('claimStatus', 'RA');
            frm.append('odIxs', odIxs.join(','))
            frm.append('ccReason', this.cancelReason);
            ...
            apiService.postFile(API_URL_UPDATE_MYORDER_EXCHANGE, frm, (res) => {
                ...
                let params = {};
                if (res.data.result == 'success') {
                    params.message = '환불 요청이 접수되었습니다';
                    params.doAction = () => {
                        this.$router.go(-2);
                    }
                } else if (res.data.result == 'already') {
                    params.message = '이미 환불요청 접수된 정보입니다';
                } else {
